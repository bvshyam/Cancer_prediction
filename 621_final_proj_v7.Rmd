...
Our main purpose is to predict if the cells (tumor) are benign or malignant (cancer), based on dimensional characteristics of the nuclei cells.  
As secondary objectives we want to know if the quality of our prediction is similar to the scientific articles and if linear models are still competitive against nonlinear models, based on rules / trees.
...
largest values) of these dimensional features were computed for each image,
...
plot(pca_wdbc, type='l', main="PCA - Principal Components Analysis Chart")
...
acc13<-conf_13_logit_t$overall["Accuracy"]
acc14<-conf_14_pca_t$overall["Accuracy"]
acc15<-conf_15_probit_t$overall["Accuracy"]

# compute AUC

auc13<-roc(test_y ~ conv_13_logit_t, test_x)$auc
auc14<-roc(test_y ~ conv_14_pca_t, test_x)$auc
auc15<-roc(test_y ~ conv_15_probit_t, test_x)$auc
...
(ROC, Confusion matrix, all metrics, residual ....)
...
```{r include=FALSE, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
#suspended code
...
```{r include=TRUE, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
#will suspend code
